...
extern Calendar_DrvTypeDef calendar;

void Calendar_init(void);
//每秒执行一次驱动时钟的运行
//GPS接收数据后执行,将GPS接收的数据更新时钟.或者选用其他时钟源，比如DS3231，DS1302等等
void Calendar_sec_process_dev(uint8_t *date,uint8_t *time,uint8_t timezone_flag,uint8_t sec_flag);//格式date:ddmmyy
																//time:hhmmss
...
void	Calendar_set_time(uint8_t *time);//1.字符串设置时间。
							 //2.GPS驱动时钟，所需要调用的子函数
void	Calendar_set_date(uint8_t *date);//1.字符串设置日期。
																//2.GPS驱动时钟，所需要调用的子函数
void	timezone_adjust(uint8_t timezone);//时区调整，自动调整日期的转换
void	add_one_sec(void);//1.用于时间的矫正。
														//2.如果系统提供秒中断服务，可使用秒中断驱动本函数，可实现整个日历的运行
void	add_one_day(void);//日期的矫正。
...
uint8_t		alarm_check(void);//检查闹钟时候到时间
uint8_t		alarm_check_only_time(void);//检查闹钟时候到时间
void	alarm_enable(uint8_t enable);//倒计时的使能
void	update_cutdown(void);//更新倒计时字符串       
 
///////以下是日历计算的私有函数///////////////////////
uint8_t 		is_leap_year(u16 _year);//是否是闰年
void	mon_1_3_5_7_8_10_12_add_one_day(void);//天的进位
void	mon_4_6_9_11_add_one_day(void);//天的进位
void	mon_2_add_one_day(void);//天的进位
