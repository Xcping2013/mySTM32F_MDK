...
static void output_write(uint8_t ch , uint8_t state)
{
	switch(ch)
	{
		...
		case 14:				
		case 15:			
		case 16:		if(state) hc595.out[1] |= (0x80>>(16-ch));		//0000 0001 ~ 1000 0000 置位设置通道
								else   		hc595.out[1] &= ((0x7f >> (16-ch)) | (0x7f << (8 -(16-ch))));//循环右移 1111 1110 ~ 0111 1111 复位设置通道
								hc595.write(&hc595.out[0],2,MSB_FIRST);				//2个595传输设置值
								hc595.updata();																//输出设置值
			break;
		...
		case 24:
							if(state==0)   hc595.out[0] |= (0x80>>(24-ch));
								else   		hc595.out[0] &= ((0x7f >> (24-ch)) | (0x7f << (8 -(24-ch))));//循环右移
								hc595.write(&hc595.out[0],2,MSB_FIRST);
								hc595.updata();
			break;
